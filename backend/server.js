  ...
  if (!fs.existsSync(dbFile)) {
    ...
    const initialData = {
      ...
      items: [
        {
          id: 1,
          name: "Furuno EPIRB Tester",
          serial_number: "EP-9921",
          category: "GMDSS / Safety",
          status: "Available",
          current_holder: "-",
        ...
        {
          id: 2,
          name: "RF Power Watt Meter",
          serial_number: "WM-4410",
        ...
        {
          id: 3,
          name: "JRC Navtex Receiver Tester",
          serial_number: "NX-8820",
          category: "Navigation",
          status: "Available",
          current_holder: "-",
          holder_username: null,
...
app.post("/api/items/:id/checkout", auth, (req, res) => {
  ...
  if (item.status === "In Use") {
    return res
      .status(400)
      .json({ error: "Item is already checked out by another user." });
  }
...
app.post("/api/items/:id/return", auth, (req, res) => {
  ...
  if (!isOwner && !isAdmin) {
    return res.status(403).json({
      error:
        "Only the user who checked out this item (or an Admin) can return it.",
    });
  }
...
app.listen(PORT, () =>
  console.log(`NavCom Inventory server running on port ${PORT}`),
);
